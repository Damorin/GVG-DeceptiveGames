package tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.heuristics;
...
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.search.ehc.EHCPlayer;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.search.waysolving.A_Star;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.search.waysolving.Grid;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.search.waysolving.GridElement;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.util.GameTypeChecker;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.util.Util;
...
public class Knowledge {
	...
	private Knowledge(StateObservation stateObs, ElapsedCpuTimer elapsedTimer, boolean catchingGame) {
		...
		if (!catchingGame) {
			...
			while (elapsedTimer.remainingTimeMillis() > Knowledge.DO_ANOTHER_ITERATION_UNTIL) {
				...
				boolean collision = !stateObs.getEventsHistory().isEmpty()
						&& stateObs.getEventsHistory().last().gameStep == lastState.getGameTick();
	...
	private boolean makeSingleMove(List<ACTIONS> actionPathToObject, ElapsedCpuTimer elapsedTimerIteration,
		...
		// test if there was a collision
		boolean collision = !currentState.getEventsHistory().isEmpty()
				&& currentState.getEventsHistory().last().gameStep == lastState.getGameTick();
	...
	public static void updateExperience(StateObservation lastState, StateObservation newState) {
		if (lastState != null && newState != null) {
			boolean collision = !newState.getEventsHistory().isEmpty()
					&& newState.getEventsHistory().last().gameStep == newState.getGameTick();
			if (collision) {
