package tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.mcts;
...
import tools.Utils;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.Agent;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.heuristics.Heatmap;
import tracks.singlePlayer.agentsForDeceptiveGames.AtheneAI.heuristics.Knowledge;
...
public class SingleTreeNode {
	...
	public void mctsSearch(ElapsedCpuTimer elapsedTimer, Heatmap map) {
		...
		while (remaining > 2 * avgTimeTaken && remaining > remainingLimit) {
			...
			double delta = selected.rollOut(state, map);
			...
			rootState.setNewSeed(m_rnd.nextInt());
		}
	}
	...
	public double rollOut(StateObservation state, Heatmap map) {
		int thisDepth = this.m_depth;
		int[] actions = new int[Agent.ROLLOUT_DEPTH];
		int numOfActions = 0;
		while (!finishRollout(state, thisDepth)) {
			if (actionChain == null) {
				...
				int chance = m_rnd.nextInt(100);
				double[] probs = actionChain.getPropabilities();
				double acc = 0;
				for (int i = 0; i < probs.length; ++i) {
					acc += probs[i];
					if (chance <= acc * 100) {
						int action = i;
						actions[numOfActions] = action;
						state.advance(Agent.actions[action]);
						thisDepth++;
						break;
					}
	...
	public double value(StateObservation a_gameState, Heatmap map, int depth) {
		...
		if (collision) {
			...
			int agentTypeId = e.activeTypeId;
			int passiveTypeId = e.passiveTypeId;
		...
		if (gameOver && win == Types.WINNER.PLAYER_LOSES)
			rawScore += HUGE_NEGATIVE / (Math.exp(depth));
