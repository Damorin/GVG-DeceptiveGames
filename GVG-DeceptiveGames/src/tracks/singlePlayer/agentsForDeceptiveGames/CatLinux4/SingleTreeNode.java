package tracks.singlePlayer.agentsForDeceptiveGames.CatLinux4;
...
public class SingleTreeNode
{
    ...
    public int selectActionWithProba() {
        double[] probas = new double[children.length];
        double maxProba = 0.0;
        for (int i=0; i<children.length; i++) {
            if(children[i] != null) {
                double childProba = children[i].nVisits/this.nVisits;
                maxProba = (maxProba > childProba) ? maxProba : childProba;
                probas[i] = childProba;
            }
        }
        double threshold = maxProba*0.7;
        double sumProba = 0.0;
        for(int i=0; i<children.length; i++) {
            probas[i] = (probas[i] > threshold) ? probas[i] : 0.0;
            sumProba += probas[i];
        }
        return selectWithProba(probas, sumProba);
    }

    public int selectWithProba(double[] probas, double sumProba) {
        double cumsum = probas[0]/sumProba;
        int idx = 0;
        double proba = this.m_rnd.nextDouble();
        while(proba > cumsum) {
            idx++;
            cumsum += probas[idx]/sumProba;
        }
        assert(idx <= probas.length);
        return idx;
    }

    public double[] cumsum(double[] probas) {
        double[] cumsum = new double[probas.length];
        cumsum[0] = probas[0];
        for(int i=1; i<cumsum.length; i++) {
            cumsum[i] = probas[i] + cumsum[i-1];
        }
        return cumsum;
    }

    public static void TruncateArray(double[] array, double threshold) {
