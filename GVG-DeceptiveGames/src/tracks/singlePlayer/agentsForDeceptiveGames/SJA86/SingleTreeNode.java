package tracks.singlePlayer.agentsForDeceptiveGames.SJA86;
...
public class SingleTreeNode
{
	...
	//added
	public boolean badRun = false;
	public static int minimum_unbalance = 5;
	public boolean alternate_on = false;
	public Vector2d original_pos;
    ...
    public SingleTreeNode(StateObservation state, SingleTreeNode parent, Random rnd) {
        ...
        if(parent != null){
			...
			alternate_on = parent.alternate_on;
			original_pos = parent.original_pos;
		}
        else{m_depth = 0;}
    }


    public void mctsSearch(ElapsedCpuTimer elapsedTimer, boolean switch_indicator, Vector2d pos) {
		
		alternate_on = switch_indicator;
		original_pos = pos;
    ...
    public double value(StateObservation a_gameState) {
        ...
        double rawScore;
		if (alternate_on) {
			Vector2d current_pos = a_gameState.getAvatarPosition();
			rawScore = current_pos.dist(original_pos);
		}
		else {rawScore = a_gameState.getGameScore();}
    ...
    public int mostVisitedAction() {
        ...
        for (int i=0; i<children.length; i++) {
            ...
            if(children[i] != null)
            {
				...
				if (childValue < leastValue) {leastValue = childValue;}
            }
        }
		...
		if (allEqual || (bestValue - leastValue) < minimum_unbalance) {badRun = true;}
		return selected;
    }
	...
	public boolean run_status () {return badRun;}
}
