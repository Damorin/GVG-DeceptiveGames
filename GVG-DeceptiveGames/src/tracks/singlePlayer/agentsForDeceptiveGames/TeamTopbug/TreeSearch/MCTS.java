...
import tracks.singlePlayer.agentsForDeceptiveGames.TeamTopbug.Agent;
		...
		while (!Agent.anyTime.isTimeOver()) {
			...
			while (selected.depth < MAX_SELECTION_DEPTH) {
				...
				if (selected.children.isEmpty()) {
					...
					break;
					// performance todo: wenn wir nach expand direkt weiter selecten kommen wir
					// tiefer,
					// dafuer kann MCTS nicht so gut entscheiden ob es sinn macht bis dahin zu
					// kommen
					// break einfach entfernen, dann selected er von hier aus weiter (innerhalb der
					// kinder)
