package tracks.singlePlayer.agentsForDeceptiveGames.YBCriber.pcollections;
 ...
 * Data.Map implementation, which in turn is based on "size balanced binary
 * trees" as described by:
 * <p>
...
class IntTree<V> {
	...
	IntTree<V> minus(final long key) {
		...
		// otherwise replace this with the next key (i.e. the smallest key to the
		// right):
		...
		// lastly, make the subtree keys relative to newKey (currently they are relative
		// to this.key):
	 ...
	 * 
	 * This method will create an _invalid_ tree if delta<0 and the distance between
	 * the smallest k>=key in this and the largest j<key in this is |delta| or less.
	 * 
	 * In other words, this method must not result in any change in the order of the
	 * keys in this, since the tree structure is not being changed at all.
	 */
	IntTree<V> changeKeysAbove(final long key, final int delta) {
		...
		if (this.key >= key)
			...
			// all of which are to the left. note that we still use the 'old' relative
			// key...:
	 ...
	 * 
	 * This method will create an _invalid_ tree if delta>0 and the distance between
	 * the largest k<key in this and the smallest j>=key in this is delta or less.
	 * 
	 * In other words, this method must not result in any overlap or change in the
	 * order of the keys in this, since the tree _structure_ is not being changed at
	 * all.
	 */
	IntTree<V> changeKeysBelow(final long key, final int delta) {
		...
		if (this.key < key)
			...
			// all of which are to the right. note that we still use the 'old' relative
			// key...:
	...
	private static <V> IntTree<V> rebalanced(final long key, final V value, final IntTree<V> left,
		...
		if (left.size + right.size > 1) {
			if (left.size >= OMEGA * right.size) { // rotate to the right
				...
				if (lr.size < ALPHA * ll.size) // single rotation
					return new IntTree<V>(left.key + key, left.value, ll,
					...
					return new IntTree<V>(lr.key + left.key + key, lr.value,
							...
							new IntTree<V>(-left.key - lr.key, value, lrr.withKey(lrr.key + lr.key + left.key), right));
				}
				...
				if (rl.size < ALPHA * rr.size) // single rotation
					return new IntTree<V>(right.key + key, right.value,
							new IntTree<V>(-right.key, value, left, rl.withKey(rl.key + right.key)), rr);
				else { // double rotation:
					IntTree<V> rll = rl.left, rlr = rl.right;
					return new IntTree<V>(rl.key + right.key + key, rl.value,
							new IntTree<V>(-right.key - rl.key, value, left, rll.withKey(rll.key + rl.key + right.key)),
