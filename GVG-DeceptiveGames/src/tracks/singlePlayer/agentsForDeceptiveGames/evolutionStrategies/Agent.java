package tracks.singlePlayer.agentsForDeceptiveGames.evolutionStrategies;
...
import tracks.singlePlayer.agentsForDeceptiveGames.Heuristics.SimpleStateHeuristic;
...
public class Agent extends AbstractPlayer {
	...
	private class Chromosome implements Comparable<Chromosome> {
		private ArrayList<ACTIONS> list;
		private double fitness;
		private StateObservation stateObs;

		public Chromosome(StateObservation stateObs, ArrayList<ACTIONS> actions, Random random, int length) {
			this.stateObs = stateObs;
			this.list = new ArrayList<ACTIONS>();
			StateObservation newState = stateObs.copy();
			for (int i = 0; i < length; i++) {
				ACTIONS act = actions.get(random.nextInt(actions.size()));
				list.add(act);
		...
		public Chromosome(StateObservation stateObs, ArrayList<ACTIONS> list) {
			this.stateObs = stateObs;
			this.list = list;
			StateObservation newState = stateObs.copy();
		...
		public Chromosome getMutation(Random random) {
			ArrayList<ACTIONS> newList = (ArrayList<ACTIONS>) list.clone();
