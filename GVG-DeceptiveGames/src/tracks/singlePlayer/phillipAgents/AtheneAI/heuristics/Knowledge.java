package controllers.AtheneAI.heuristics;
...
import controllers.AtheneAI.search.waysolving.A_Star;
import controllers.AtheneAI.search.waysolving.Grid;
import controllers.AtheneAI.search.waysolving.GridElement;
 ...
 * TODO: check everywhere for NPEs etc.
 * TODO: check for waysolving over movables (see boloadventures level 0: should reach portal and vulcano here!!)
 */
public class Knowledge {
	...
	// number of events that should be detected (size of an experience array for each type)
	...
	private Knowledge(StateObservation stateObs, ElapsedCpuTimer elapsedTimer, boolean catchingGame) {
    	...
    	// getting this here addresses cases where portals disappear (TODO: however the case that portals appear in the end is not handled!)
    	...
    	if (!catchingGame){
        	...
        	while (elapsedTimer.remainingTimeMillis() > Knowledge.DO_ANOTHER_ITERATION_UNTIL){
        		...
        		if (exploreObjects){
        			...
        			// this will cause the avatar to do NIL, so that we are able to further monitor the gamestate.
				...
				for (ArrayList<Observation> portalType : portals) {
					if (portalType != null && !portalType.isEmpty()) {
						if (spawners.containsKey(portalType.get(0).itype)){
							spawners.put(portalType.get(0).itype, Math.min(1.0, (spawners.get(portalType.get(0).itype)/portalType.size())/this.spawnerDetections));
						} else{
    		...
    		while (elapsedTimer.remainingTimeMillis() > Knowledge.DO_ANOTHER_ITERATION_UNTIL){
    			...
    			ACTIONS action = ehcPlayer.getBestAction(stateObs, elapsedTimer, getInterestingMovables(stateObs, agentType));
    			StateObservation lastState = stateObs.copy();
    			stateObs.advance(action);
    			gameTicksSimulated++;
    			
    			// test if there was a collision
    	        boolean collision = !stateObs.getEventsHistory().isEmpty() && stateObs.getEventsHistory().last().gameStep == lastState.getGameTick();
    	        
    			if (collision) {
    				Event e = stateObs.getEventsHistory().last();
	...
	/**
	 * Selects next object to chase and determines how to reach it.
	 * Returns false if there are no more objects found to chase or not enough time left, otherwise true.
	 */
	private boolean doAnotherIteration(){
	    ...
	    if (debug){
	    	System.out.println("************** starting new iteration (gametick = " + currentState.getGameTick() + ", simulated tick = " + this.gameTicksSimulated + ") **************");
		...
		if (mostInterestingObservation != null){
			if (debug)
				System.out.println("most interesting observation is typeID=" + mostInterestingObservation.itype + " obsID="+ mostInterestingObservation.obsID);
			
			if (mostInterestingObservation.category != Knowledge.CATEGORY_NPC && timer.remainingTimeMillis() > aStarAvgTime){
			
				Vector2d startPos = currentState.getAvatarPosition();
				Vector2d targetPos = mostInterestingObservation.position;
				if (aStar.solve((int)(startPos.x/blockSize), (int)(startPos.y/blockSize), (int)(targetPos.x/blockSize), (int)(targetPos.y/blockSize), true, false)){
					actionPathToObject = aStar.getActionSequence(GameTypeChecker.doubleMovesNeeded(currentState), Util.getDirection(currentState.getAvatarOrientation()));
					if (debug)
					...
					setObservationUnreachable(agentType, mostInterestingObservation.itype, mostInterestingObservation.obsID);
					return true;
				}
			}
		
			// do EHC if interesting movable id found and time is sufficient
			else if (mostInterestingObservation.category == Knowledge.CATEGORY_NPC && timer.remainingTimeMillis() > ehcAvgTime){
				if (debug)
		...
		// nothing more to explore or not enough time
			...
			if (debug)
				System.out.println("nothing interesting to explore anymore, simulated gameticks so far = " + gameTicksSimulated);
			return false;
		}
	...
	/**
	 * Makes a single move and identifies collisions.
	 * Returns false if the current target should be abandoned and another object should be chased, otherwise true.
	 */
	private boolean makeSingleMove(List<ACTIONS> actionPathToObject, ElapsedCpuTimer elapsedTimerIteration, boolean justDoNil){
		boolean lastMoveOnPath = false;
		int agentType = Util.getAgentType(currentState);
		
		// select a move to make
		ACTIONS action = ACTIONS.ACTION_NIL;
		if (!justDoNil && mostInterestingObservation != null && mostInterestingObservation.category == Knowledge.CATEGORY_NPC) {
		...
		// if it was the last move on the current A* path, check if we reached the target and update waycosts
		...
		if (lastMoveOnPath){
			...
			// clear unreachable lists if an immovable id assumed to be blocked has become free
			if (getWayCosts(agentType, mostInterestingObservation.itype) == Knowledge.WAY_COSTS_BLOCKED && newWayCosts != Knowledge.WAY_COSTS_BLOCKED){
				if (debug)
        ...
        // test if there was a collision
        boolean collision = !currentState.getEventsHistory().isEmpty() && currentState.getEventsHistory().last().gameStep == lastState.getGameTick();
		...
		if (collision || steppedOnTarget) {
			...
			Event e;
			if (collision){
				e = currentState.getEventsHistory().last();
				collidedWithTypeID = e.passiveTypeId;
			} else{
				collidedWithTypeID = mostInterestingObservation.itype;
				e = new Event(lastState.getGameTick(), false, agentType, collidedWithTypeID, 0, mostInterestingObservation.obsID, lastState.getAvatarPosition());
			}
			if (debug)
				System.out.println("there was a collision with typeID=" + collidedWithTypeID);
			...
			if (!justDoNil && mostInterestingObservation != null && mostInterestingObservation.category == Knowledge.CATEGORY_NPC && collidedWithTypeID == mostInterestingObservation.itype){
				return false;
			}
		}
	...
	/**
     * Heuristic for walking towards the nearest interseting npc of the current gamestate.
     */
			...
			if (npcs != null) {
				for (ArrayList<Observation> npcType : npcs) {
					if (npcType != null && !npcType.isEmpty() && npcType.get(0).itype == typeID) {
						int distance = Util.manhattanDistInBlocks(stateCopy.getAvatarPosition(), npcType.get(0).position, blockSize);
	...
	/**
	 * Call this regulary to detect spawners. Spawners are "bad" portals so we should not step on 
	 * them and should not assume them to be exits later in the real game. Here only the frequency 
	 * of being detected as a spawner for each portal type is accumulated. At the end of init, this 
	 * is transformed into a probability for each portal type to be a spawner.
	 * 
				...
				if (portalType != null && !portalType.isEmpty()) {
					for (Observation portal : portalType){
						...
						if (observationsOnPortalState2.size() > 1){
							...
							//System.out.println("things on portal itype=" + portal.itype + ", position=" + portal.position + ":");
							for (Observation o : observationsOnPortalState2){
								...
								if (o.category == Knowledge.CATEGORY_MOVABLE){
									//System.out.println("(movable) itype=" + o.itype + ", obsID=" + o.obsID + ", position=" + o.position);
											...
											if (movType != null && !movType.isEmpty()) {
												...
												for (Observation mov : movType){
													if (o.obsID == mov.obsID){
														//System.out.println("found in earlier state, position=" + mov.position);
									...
									//System.out.println("(npc) itype=" + o.itype + ", obsID=" + o.obsID + ", position=" + o.position);
									if (npcsState1 != null) {
											...
											if (npcType != null && !npcType.isEmpty()) {
												...
												for (Observation npc : npcType) {
													if (o.obsID == npc.obsID) {
														//System.out.println("found in earlier state, position=" + npc.position);
	...
	/**
	 * Call this regulary to update waycosts for dynamic movables which are appearing at some point in time. 
	 * Otherwise the waycosts for these movables will not be known and therefore assumed to be BLOCKED.
	 */
    ...
    private void getMostInterestingObservation(StateObservation stateObs, int agentType) {
		...
		if (immovables != null) {
			for (ArrayList<Observation> immovableType : immovables) {
				if (immovableType != null && !immovableType.isEmpty() && !isTypeUnreachable(agentType, immovableType.get(0).itype)) {
		...
		if (portals != null && leastCollisions != 0) {
			for (ArrayList<Observation> portalType : portals) {
				if (portalType != null && !portalType.isEmpty() && !isTypeUnreachable(agentType, portalType.get(0).itype)) {
		...
		if (movables != null) {
			for (ArrayList<Observation> movableType : movables) {
				if (movableType != null && !movableType.isEmpty()) {
					if (this.getNrOfCollisions(agentType, movableType.get(0).itype) < Knowledge.NR_COLLISIONS_TO_BE_UNINTERESTING) {
	...
	/**
	 * Returns the learned way costs for a field with the given Immovable type on it for 
	 * an agent with the given agent id. Assumes field is blocked for unknown types.
	 */
	...
	/**
	 * Updates the wayCosts for a field with the given Immovable type on it for 
	 * an agent with the given agent id.
	 */
	...
	/**
	 * Adds a unique obsID to be unreachable. If x unique obsIDs of the same type are 
	 * unreachable, sets the whole type to be unreachable.
	 */
	private void setObservationUnreachable(int agentType, int spriteType, int obsID){
		...
		if (unreachableObsIDs.get(agentType).containsKey(spriteType)){
			if (unreachableObsIDs.get(agentType).get(spriteType).size() >= NR_OBS_UNREACHABLE_FOR_TYPE_UNREACHABLE-1){
	...
	/**
	 * Returns true if an observation is unreachable for a given agent type, otherwise false.
	 */
	...
	/**
	 * Sets complete type unreachable for a given agent id, so that the agent will not try to reach objects 
	 * of this type again before the list is cleared.
	 */
	...
	/**
	 * Returns true if spriteType is unreachable for a given agent type, otherwise false.
	 */
	...
	/**
	 * Remembers how many times an agent with the given id collided with a sprite with the given id.
	 */
	...
	/**
	 * Returns how many times an agent with the given id collided with a sprite with the given id.
	 */
	...
	private static void printExperience() {
		...
		for (int agent: experience.keySet()) {
			...
			System.out.println("Sprite Type\tWin\t\tLoss\t\tScore Down\tScore Up\tInventory Down\tInventory Up\tNew Immov\tNew Resource");
	...
	/**
	 * Update the experience after a new collision. A collision must have happened directly in the lastState, otherwise no update happens
	 * @param lastState - state where a collision happened
	 * @param newState - state right after collision
	 */
	public static void updateExperience(StateObservation lastState, StateObservation newState) {
		if (lastState != null && newState != null) {
			boolean collision = !newState.getEventsHistory().isEmpty() && newState.getEventsHistory().last().gameStep ==
					newState.getGameTick();
			if (collision) {
				evaluateCollision(lastState, newState, newState.getEventsHistory().last());
			}
		}
	}
	
	/**
	 * Here the changes from last to new game state are evaluated 
	 * and a new value is assigned to the object with whom the agent collided.
	 */
	...
	/**
	 * Here we look for changes in two given states. May be extended with further change types.
	 */
	private static boolean[] detectChanges(StateObservation lastState, StateObservation newState){
		...
		changed[Knowledge.LOSS] = newState.getGameWinner() == Types.WINNER.PLAYER_LOSES && newState.getGameWinner() != lastState.getGameWinner();
		changed[Knowledge.WIN] = newState.getGameWinner() == Types.WINNER.PLAYER_WINS && newState.getGameWinner() != lastState.getGameWinner();
		...
		changed[Knowledge.INVENTORY_DECREASED] = inventoryDecreased(lastState.getAvatarResources(), newState.getAvatarResources());
		changed[Knowledge.INVENTORY_INCREASED] = inventoryIncreased(lastState.getAvatarResources(), newState.getAvatarResources());
		changed[Knowledge.NEW_IMMOV_TYPE] = detectNewTypes(lastState.getImmovablePositions(), newState.getImmovablePositions());
		changed[Knowledge.NEW_RES_TYPE] = detectNewTypes(lastState.getResourcesPositions(), newState.getResourcesPositions());
		
		return changed;
	}
	
	/**
	 * Returns wether the inventory of the agent has increased from last to new state.
	 */
	private static boolean inventoryIncreased(HashMap<Integer, Integer> lastStateInventory, HashMap<Integer, Integer> newStateInventory) {
	 ...
	 * Returns wether the inventory of the agent has increased from last to new state.
	 */
	private static boolean inventoryDecreased(HashMap<Integer, Integer> lastStateInventory, HashMap<Integer, Integer> newStateInventory) {
	 ...
	 */
	private static boolean detectNewTypes(ArrayList<Observation>[] lastStateArray, ArrayList<Observation>[] newStateArray) {
	 ...
	 * @param agentTypeId
	 * @return experienceMap - mapping from passive objects to experience arrays, for a given agent type
	 */
	 ...
	 * @param agentTypeId 
	 * @param passiveTypeId
	 * @return experienceArray - an array where indices are the events and values mean, 
	 * how often this event happened after the collision of the agent of given type 
	 * with a passive object of given type.
	 * If agent or passive object are not known - returns null.
	 */
	...
	/**
	 * for a given state returns a list of observations (immovables, movables, npc etc), 
	 * that have only one instance of their type in the state
	 * and that haven't been seen before (not present in the experience map)
	 * @param state - current state, for which the list is evaluated
	 * @param agentTypeId - type of an agent whose experience we consider
	 * @return a list of observations, that have only one instance of their type 
	 * and are not known to the given agent
	 * The list is empty, if no objects satisfying the criteria are found
	 */
	...
	/**
	 * for a given list of observations (structure returned by getNPCPositions and similar functions)
	 * returns Observations (objects), that exist in one instance only (alone)
	 * @param observations 
	...
	/**
	 * Returns "benefit" of colliding with a passive object. 
	 * Use this method if you do not know the agentType.
	 * Benefit is between -1 (bad) and +1 (good), 0 = unknown.
	 */
	...
	/**
	 * Returns a list of movable ids that are considered to be good based on knowledge.
	 */
	...
	/**
	 * Movable (or any other observation) is considered good if nothing bad happens when we collide with it
	 * or if we don't know anything about it (better to take risk than miss a potentially good object)
	 */
	...
	/**
	 * Call this to determine the probabilities of portals and immovable types to be an exit. 
	 * This depends on the agent type do you have to call it regulary with the current type.
	 */
	...
	/**
	 * Call this to (re-)calculate the distances of all fields to the 'goal' for the given game state. 
	 * Returns true if a 'goal' was found and you can use the heuristik by calling getDistanceHeuristicValue(...).
	 */
	public static boolean updateDistanceHeuristic(StateObservation state, ElapsedCpuTimer elapsedTimer, int limit){
		...
		if (goal[0] >= 0 && goal[1] >= 0){
			aStar.solve(goal[0], goal[1], (int)state.getAvatarPosition().x/blockSize, (int)state.getAvatarPosition().y/blockSize, true, true);
			grid = aStar.getGrid();
	...
	/**
	 * Call to get the heuristic value for a given position. The returned value will be in [0,1], 
	 * where 0 means the field is far away from the 'goal' and 1 means the 'goal' is at the given position. 
	 * If there is no path found to the 'goal' the approximate distance (manhattan) will be used for computing a value.
	 */
	...
	private static void printValues() {
		...
		for (GridElement[] array : grid.grid) {
			...
			for (GridElement element : array) {
				double d = getDistanceHeuristicValue(new Vector2d(element.getRow() * blockSize, element.getColumn()* blockSize));
