package controllers.AtheneAI.search.ehc;
...
/**
 * Chooses the next move by using Enforced Hill Climbing. 
 * Apply for games where the agent only has two walking directions and there are movables falling down to catch. 
 * The next move is chosen by minimizing the distance of the agent to the movable that is nearest to the ground.
 * 
 * Agent is not applied, if there is evidence that movable could do any harm.
 * 
 * Ground can be on any side of the play field. It is determined, if ground is horizontal or vertical.
 * 
 * TODO: it should be checked if movables are really falling down and not just lying somewhere the agent can not reach them. 
 * For example we could recognize if the distance to an object never decreases and then remember this object to be unreachable. 
 * If there only unreachable objects MCTS should be applied.
 * 
 * TODO: In contrast to aliens in eggomania the agent with olmcts is not able to kill the npc. Find out the reason for that and maybe we 
 * can adapt Simons player. (However for eggomania we win anyway. And it would be rather bad if we directly kill the npc because then we 
 * would catch less coins. But for other games it might be a requirement for winning to kill the npc)
 * 
 * TODO: we should update Knowledge also here if we get to know that sth. is good/bad!
 * 
...
public class EHCPlayer {
	...
	/**
	 * Returns the action considered to be best according to EHC. 
	 * This means the best move is chosen by minimizing the distance 
	 * of the agent to the movable that is nearest to the ground.
	 */
	public ACTIONS getBestAction(StateObservation state, ElapsedCpuTimer timer, ArrayList<Integer> goodMovablesIds) {
		...
		if (movables != null) {
			...
			if (obsNearestToGround != null){
				...
				if (debug)
					System.out.println("obsId = " + obsNearestToGround.obsID + " is " + nearestDistanceToGround + "px away from ground");
				...
				for (ACTIONS a : state.getAvailableActions(true)){
					...
					if (a != ACTIONS.ACTION_USE && a != ACTIONS.ACTION_ESCAPE){
						...
						if (!(stateCopy.isGameOver() && stateCopy.getGameWinner() == Types.WINNER.PLAYER_LOSES)){
							// NOTE: by advancing the state the position in the movable object is NOT updated, but it should work fine anyway
				...
				if (debug)
					System.out.println("action = " + bestAction + " is best action, distance to agent is now " + nearestDistanceToAgent + "px");
			}
		}
	...
	/**
	 * Returns distance in pixel between two points along one axis. Axis is orthogonal to the ground.  
	 */
    private int distToGround(Vector2d avatarPosition, Vector2d observationPosition){
    	...
    	else return (int) Math.abs((avatarPosition.x - observationPosition.x));
	}
    ...
    private int learnGroundOrientation() {
    	...
    	else return EHCPlayer.VERTICAL;
    }
