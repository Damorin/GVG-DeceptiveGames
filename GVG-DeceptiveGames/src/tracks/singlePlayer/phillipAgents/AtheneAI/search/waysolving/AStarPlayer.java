...
public class AStarPlayer {
	...
	/**
	 * Returns a new action plan (i.e. a list of actions to go to reach an interesting object). 
	 * In this method AStarPlayer autonomously determines what is the most interesting object.
	 */
	public List<ACTIONS> getNewActionPlan(StateObservation state, ElapsedCpuTimer timer, int agentType) {
		...
		// try to find a path
		if (debug)
			System.out.println("trying to find path from (" + myPosition[0] + "," + myPosition[1] + ") to (" + currentTarget[0] + "," + currentTarget[1] + ")");
	...
	private void whereToGoNext(StateObservation stateObs) {
		...
		if (bestObservation != null){
			if (debug)
				System.out.println("nearest interesting observation is itype=" + bestObservation.itype + ", obsID=" + bestObservation.obsID);
