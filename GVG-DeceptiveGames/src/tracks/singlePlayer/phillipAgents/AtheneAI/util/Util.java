...
public class Util {
    ...
    public static int manhattanDistInPx(Vector2d startPosition, Vector2d targetPosition){
    	return (int) (Math.abs(startPosition.x - targetPosition.x) 
    			+ Math.abs(startPosition.y - targetPosition.y));
	}
    	...
    	if(positions != null && positions.length != 0){
            ...
            for (int i = 0; i < positions.length; i++) {
            	if (!positions[i].isEmpty())
            		System.out.print(positions[i].get(0).itype + " (category=" + positions[i].get(0).category + "):" + positions[i].size() + ",");
            }
            System.out.print("); ");
	...
	/**
	 * using the known position of the agent and the fact, that agents belong to the 0 category,
	 * extract the agent Id from the grid (Maria)
	 */
	public static int getAgentType(StateObservation state) {
    	...
    	ArrayList<Observation> observations = state.getObservationGrid()
    			[(int)position.x/state.getBlockSize()][(int) position.y/state.getBlockSize()];
